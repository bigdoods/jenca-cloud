...
CONTROLLER_CLOUD_CONFIG_PATH = File.expand_path("scripts/generic/controller-install.sh")
WORKER_CLOUD_CONFIG_PATH = File.expand_path("scripts/generic/worker-install.sh")
...
# Generate root CA
system("mkdir -p ssl && ./scripts/lib/init-ssl-ca ssl") or abort ("failed generating SSL artifacts")

# Generate admin key/cert
system("./scripts/lib/init-ssl ssl admin kube-admin") or abort("failed generating admin SSL artifacts")

def provisionMachineSSL(machine,certBaseName,cn,ipAddrs)
  tarFile = "ssl/#{cn}.tar"
  ipString = ipAddrs.map.with_index { |ip, i| "IP.#{i+1}=#{ip}"}.join(",")
  system("./scripts/lib/init-ssl ssl #{certBaseName} #{cn} #{ipString}") or abort("failed generating #{cn} SSL artifacts")
